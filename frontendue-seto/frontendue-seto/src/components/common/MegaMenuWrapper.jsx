...
export default function MegaMenuWrapper() {
  ...
  return (
    <nav
      className={`fixed w-full z-[999] transition-all duration-500 ease-in-out ${isScrolled
        ? 'bg-hero-gradient shadow-lg py-2'
        : 'bg-transparent py-6'
        }`}
    >
        ...
        <div className="flex items-center justify-between">
          ...
          <a href="/" className="flex items-center gap-2 group transition-transform hover:scale-105 duration-300">
            ...
            <div className="flex flex-col border-l border-white/20 pl-3 ml-1">
              <span className={`font-heading font-black leading-none text-lg tracking-tighter text-white`}>ULTIMATE</span>
              <span className={`font-heading font-medium leading-none text-[10px] tracking-[0.2em] uppercase ${isScrolled ? 'text-white/60' : 'text-white/60'}`}>Education</span>
            </div>
          </a>
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <a
                    href={item.href}
                    className={`text-sm font-bold font-heading uppercase tracking-wide transition-all duration-300 transform hover:-translate-y-1 flex items-center gap-1 ${isScrolled
                      ? isActive ? 'text-ultimate-yellow' : item.label === 'Artikel' ? 'text-white hover:text-white' : 'text-white/80 hover:text-ultimate-yellow'
                      : isActive ? 'text-ultimate-yellow' : item.label === 'Artikel' ? 'text-white hover:text-white' : 'text-white hover:text-ultimate-yellow'
                      }`}
                    onClick={(e) => {
                      if (item.hasDropdown) {
                        e.preventDefault();
                  ...
                  <span className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 rounded-full transition-all duration-300 ${isActive ? 'w-1/2 opacity-100' : 'w-0 group-hover:w-1/2 opacity-0 group-hover:opacity-100'
                    } ${isScrolled ? 'bg-ultimate-yellow' : 'bg-ultimate-yellow'}`}></span>

                  {/* Mega Menu Dropdown */}
                  {item.hasDropdown && isMenuOpen && (
                    <div className="absolute top-full left-1/2 -translate-x-1/2 pt-5 w-[90vw] max-w-7xl z-[1000] animate-in fade-in slide-in-from-top-4 duration-300">
                ...
                <div key={item.label}>
                  ...
                  {item.hasDropdown && (
                    <div className="pl-4 border-l-2 border-white/10 mt-1 flex flex-col gap-2">
                      ...
                      <a href="/persiapan-ielts" className="text-sm py-2 text-slate-400">Ultimate IELTS</a>
                      <a href="/beasiswa" className="text-sm py-2 text-slate-400">Beasiswa</a>
                    </div>
                  )}
                </div>
              );
